...
- `inputReadonly` - отключает возможность выбора вариантов через ввод текста (autocomplete)
...
````
- `title`: заголовок диалога. Выводит в том числе и иконку закрытия диалога, если указан (в т.ч. и если пустая строка).
-  `handleClose`: обработка закрытия диалога. Можно не указывать - тогда обработка закрытия будет создана автоматически. 
Можно указать функцию которая ничего не делает, тогда диалог нельзя будет закрыть(через иконку, клик на оверлей, Esc). 
