...
при расширении форм у поля - массива `elements` есть методы:
- `get(id)` - выполняn рекурсивный поиск и возвращающий элемент по его `id`.
- `set(id, element)` - выполняn рекурсивный поиск и заменяет элемент по его `id`.
- `cut(id)` - вырезает элемент по id и возвращает его.
...
Этот способ является предпочтительным для модификации
форм родительского приложения и автоматически сгенерированных форм.
